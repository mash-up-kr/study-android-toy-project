    ...
    fun getRepository(userName: String, repoName:String){
        RetrofitService.getService().requestGetRepository(userName,repoName).enqueue(object: Callback<GithubDetailRepoData>{
            override fun onFailure(call: Call<GithubDetailRepoData>, t: Throwable) {
                ...
                activity.tv_activity_repository_detail_ErrorTextViewRepository.text = activity.getString(R.string.error) + t.toString()
                activity.tv_activity_repository_detail_ErrorTextViewRepository.visibility = VISIBLE
            }
            override fun onResponse(call: Call<GithubDetailRepoData>, response: Response<GithubDetailRepoData>) {
                val repoData=response.body()
                if (repoData != null) {
                    Glide.with(activity).load(repoData.owner.avatarUrl).into(activity.iv_activity_repository_detail_GitAvatarImage)
                    activity.tv_activity_repository_detail_FullName.text = repoData.fullName
                    activity.tv_activity_repository_detail_Stars.text = repoData.stargazersCount.toString() + activity.getString(R.string.stars)
                    activity.tv_activity_repository_detail_Description.text = repoData.description
                    if (repoData.language==null){
                        activity.tv_activity_repository_detail_Language.text = activity.getString(R.string.no_language)
                    } else {
